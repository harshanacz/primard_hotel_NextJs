    ...
    >
      <div className="flex items-center h-full justify-between w-[95%] sm:w-[90%] mx-auto">
        ...
        <div className="hidden sm:flex items-center justify-end space-x-10 w-full">
          ...
          <div className="flex items-center space-x-10">
            {navItems.map((item, index) => (
              <Link
                ...
                className="text-white font-light text-base hover:text-yellow-400 transition-all duration-300"
              >
                {item.label}
              </Link>
            ))}
          ...
          {/* Book Now Button */}
          <button
            className={`bg-yellow-400 text-black px-4 py-1.5 rounded-full text-base font-normal ${
              navBg ? "" : "hidden"
            }`}
          >
            Book Now
        ...
        <div
          className={`flex sm:hidden items-center space-x-4 `}
        >
          <button className={`bg-yellow-400 text-black px-3 py-1 rounded-full text-sm font-normal ${navBg ? "" : "hidden"}`}>
  Book Now
</button>
